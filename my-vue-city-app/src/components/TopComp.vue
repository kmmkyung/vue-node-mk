<template>
    <header>
        <ul class="gnb">
            <!--  v-값,i-속성명 -->
            <!-- 리액티브 데이터인 뷰엑스 스토어 cityData 변수를 컴포넌트 변수인 sdata에 -->
            <!-- 힐당하여 사용하면 처음에만 할당된 변수로 셋팅이되고 cityData가 변경될때 반영되지 않는다! -->
        ...
        <!-- 메뉴선택이동링크 -->
        <div class="m2">
            <!-- v-on:이벤트명.prevent : 기본기능막기 옵션
                .prevent는 event.preventDefault()와 같다! -->
            <!-- v-on:이벤트명.stop : 이벤트 버블링 막기 옵션
                .stop는 event.stopPropagation()와 같다! -->
...
export default {
    ...
    methods: {
        ...
        chgMenu(n) {
            ...
            // 메뉴변경시 DOM이 변경되므로 제이쿼리 메서드 호출하기
            // 단, 제이쿼리 코드블록으로 써서 호출함으로 DOM로드후 실행 보장!!!
            $(()=>this.setJQ());
        },
        // 제이쿼리 셋팅 메서드
        setJQ() {
